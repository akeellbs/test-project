...
class NeetDataLogics
{
    ...
    private static function data_entry_complete($path, $paper_id, $test_id, $test_class, $test_name){
        ...
        $subjects    = (AppLogicsUtils::return_specific_array($excel_arr, array(strtolower(MyAppConstants::SUB_KEY)), true));
        ...
        foreach ($roll_no_arr as $roll_nos){
            ...
            for ($i=0; $i<count($QuestionsNos); $i++){
                ...
                if ($student_ans === "BLANK"){
                    ...
                    /*echo "<br>";
                    echo "not"."[".$i."+ans=".$student_ans."]quest=".$question_no."";
                    echo "<br>";*/
                }
            ...
            /*$omr_upload_id = DB::table('tbl_test_analysis')->insertGetId([
                'testid' =>  $test_id,
                'qno' => $key,
                'correct_attempt' => $rightquestionsno,
                'in_correct_attempt' => $key,
                'not_attempt' => $key,
                'level' => $key
            ]);*/
